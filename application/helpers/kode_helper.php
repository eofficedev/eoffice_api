<?php
	...
	function generate_nomor_nota($nota){
		...
		$data;
		$i = 1;
$monthRome = ["I","II",'III','IV','V',"VI","VII","VIII","IX","X","XI","XII"];
		foreach ($konf as $kon) {
			switch($kon->tipe){
				case "auto increment": $data[$kon->id_format_code] = auto_increment();
									break;
				case "nama opd":$data[$kon->id_format_code]=$emp->org_code;
									break;
				case "kode organisasi":$data[$kon->id_format_code]=$emp->org_idd;
									break;
				case "text":$data[$kon->id_format_code]=$kon->value ;
							break;
				case "klasifikasi surat":$data[$kon->id_format_code] = $nota->nota_kode_masalah;
									break;
				case "tanggal":$data[$kon->id_format_code]=date("d");break;
				case "bulan":$data[$kon->id_format_code]=date("m");break;
				case "bulan - romawi":$data[$kon->id_format_code]=$monthRome[date("m")-1];break;
				case "tahun":$data[$kon->id_format_code]=date("Y");break;
			}
			$data[$kon->id_format_code].=$kon->delimiter;
		}
		$me->load->model('notadinas/database',"nota_nomor",true);
		$me->nota_nomor->set_table("nota_nomor");
		$me->nota_nomor->set_where(array("nota_id"=>$nota->nota_id));
		$me->nota_nomor->hapus();		
		...
		foreach ($conf as $no) {
			$kode = $kode. " " . $no->value;
			// if ($no->delimiter==0 and count($conf)>$i) $kode =$kode ." ". $delimiter; 
			$i++;	
		}
