...
class Admin_config extends CI_Model {
    ...
    function upload_anggaran() {
        ...
        if(count($q->result())>0)
            $q = $this->db->update('detail_anggaran',$data);
        else 
            $q = $this->db->insert('detail_anggaran',$data);
        $this->db->select("*");
        $this->db->from("sppd_anggaran");
        $q = $this->db->get();
        $data = array("amount"=>$this->input->post("Anggaran"),"year"=>$this->input->post("Years"));
        if(count($q->result())==0){
            $q = $this->db->insert('sppd_anggaran',$data);
        }
        else{
            $d = $q->result();
            $this->db->where("year",$d[0]->year);
            $q = $this->db->update('sppd_anggaran',$data);
    ...
    function get_anggaran(){
        $this->db->select("*");
        $this->db->from("detail_anggaran");
        $this->db->join("sppd_anggaran","detail_anggaran.years = sppd_anggaran.year");
        $q = $this->db->get();
        return $q->result();
    }
