...
class Employee extends CI_Model {
    ...
    function get_pimpinan(){
        ...
        $this->db->where('C.kepala_job_num = A.emp_job');
        $this->db->where('C.org_sub IS NULL');
        $this->db->join('hrms_job as B', 'A.emp_job=B.job_num');
        $this->db->join('hrms_organization as C', 'C.org_num=A.org_id');
        $empOrg = $this->db->get();
        return $empOrg->result()[0];
    }
    function get_sub_emp($org_id){
        $this->db->select('*');
        $this->db->from('hrms_employees as A');
        ...
        $this->db->where('A.emp_id <>', 0000000);
        $this->db->where('A.emp_id <>', 9998);
        $this->db->where('C.org_sub', $org_id);
        $this->db->where('C.kepala_job_num = A.emp_job');
        $this->db->join('hrms_job as B', 'A.emp_job=B.job_num');
        $this->db->join('hrms_organization as C', 'C.org_num=A.org_id');
        $empSubOrg = $this->db->get();
        $emp = array();
        foreach ($empOrg->result() as $r) {
            $r->kepala = false;
            $emp[] = $r;
        }
        foreach ($empSubOrg->result() as $r) {
            $r->kepala = true;
            $emp[] = $r;
        }
        return $emp;
    ...
    function get_detail_emp($username) {
        $this->db->select('A.emp_num,A.emp_id as id_emp, A.emp_job, A.pic_url,A.email_password,A.email_username,A.emp_firstname,A.emp_lastname,A.job_code,A.org_code,A.org_id,B.emp_role as emp_role,B.emp_username,C.org_id as org_idd, C.org_city');
        $this->db->from('hrms_employees as A');
    ...
    function get_detail_emp_bynum($empnum) {
    ...
    function get_all_emp() {
        ...
        $this->db->order_by('A.emp_firstname', 'ASC');
        $q = $this->db->get();
    ...
    function add_employee() {
        ...
        $data["emp_dob"] = strtotime($data["emp_dob"]);
        $data["emp_dob"] = date("Y-m-d",$data["emp_dob"]);
    ...
    function get_employee_data_by_username() {
        ...
        $this->db->select("emp_num,emp_firstname,emp_lastname,emp_gender,emp_dob,emp_street,emp_email,pic_url");
        $this->db->from("hrms_employees");
