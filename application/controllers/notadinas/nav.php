	...
	class nav extends ci_controller{
		...
		function compose($error=null){
              $this->penerima->tampil();
                 $this->data["masalah"] = $this->kode_masalah->tampil();
                  ...
                  $this->data["kop"] = generateKop($emp_data->emp_num);
                  $this->data['error'] = $error;
                  $this->data['fiatur'] = $this->employee->get_fiatur_by_org($emp_data->org_id);
                  $this->load->view("content/notadinas/compose",$this->data);
            }
                  ...
                  for($i=0;$i<count($pemeriksa);$i++){
                     ...
                     if($i+1 == 1) {
                      $this->pemeriksa->status = "AKTIF";
                        ...
                        $this->notifikasi->notif_desc = "Surat Dinas Dengan ID ".$nota_id." Perlu Di Proses";
              ...
              if($pemeriksa[0]->exam_queue==$jml_pemeriksa-1){
                ...
                foreach ($pem as $p) {
                  if($p->external == 0){
                      ...
                      $this->notifikasi->notif_desc = "Surat Dinas Masuk,ID ".$nota_id."";
                      $this->notifikasi->set_values();
                      $this->notifikasi->simpan();
                   }
                }
           ...
           public function progress_return($examine_id){
              ...
              $pem = $this->pemeriksa->tampil();
                 $this->notifikasi->notif_link = $nota_id;
                        ...
                        $this->notifikasi->notif_type = "7";
                        $this->notifikasi->notif_desc = "Surat Dinas Telah Di return, ID ".$nota_id." oleh ".$this->aktif_user->emp_firstname."/".$this->aktif_user->job_code."/".$this->aktif_user->org_code;
                        $this->notifikasi->set_values();
                        $this->notifikasi->simpan();
           ...
           public function progress_reject($examine_id){
              ...
              $nota_id = $p->nota_id;
                 $this->notifikasi->notif_link = $p->nota_id;
                        ...
                        $this->notifikasi->notif_type = "7";
                        $this->notifikasi->notif_desc = "Surat Dinas Telah Di Reject, ID ".$nota_id." oleh ".$this->aktif_user->emp_firstname."/".$this->aktif_user->job_code."/".$this->aktif_user->org_code;
                        $this->notifikasi->set_values();
                        $this->notifikasi->simpan();
              }
      ...
      function upload(){
        ...
        for ($i = 0; $i < $cpt; $i++) {
          ...
          function get_emp_sub_dir($org_id){
            echo json_encode($this->employee->get_sub_emp($org_id));
          }
          function get_emp_det($empnum){
            echo json_encode($this->employee->get_detail_emp_bynum($empnum));
          }
         ...
         $tindakan = $data["nota_tindakan"];
                 for($i=0;$i<count($kepada);$i++){
                   ...
                   $this->folder_mapping->simpan();
                        ...
                        $this->notifikasi->notif_type = "8";
                        $this->notifikasi->notif_desc = "Surat Dinas Masuk,ID ".$nota_id." Disposisi dari ".$this->aktif_user->emp_firstname."/".$this->aktif_user->job_code."/".$this->aktif_user->org_code;
                        $this->notifikasi->set_values();
                        $this->notifikasi->simpan();
      ...
      function generate_nomor($nota_id){
          $where=array("nota_id"=>$nota_id);
          $this->nota->set_where($where);
          $nota=$this->nota->tampil();
          echo generate_nomor_nota($nota[0]);
